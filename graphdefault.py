...
for r in [-6, -4, -2, 2, 4, 6]:
    ...
    if __name__ == '__main__':
        ...
        mean, anees= localize(env,policy,filt,initial_mean,args.num_steps, args.plot)
        x = np.append(x, [r])
        means = np.append(means, [mean])
        aneess = np.append(aneess, [anees])

print(x, means, aneess)
plt.plot(x, means, 'o-r')
plt.plot(x, aneess, 'o-g')
plt.legend(["Mean position error","ANEES"])
plt.xlabel("log2(r)")
plt.ylabel("Mean position error and ANEES")
